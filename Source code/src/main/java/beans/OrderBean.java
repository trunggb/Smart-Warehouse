...
public class OrderBean implements Serializable {
	private static final int NUM_PROD = 10;
	/**
	 * 
	 */
	...
	public void onClickCreateOrder(){
		...
		orderDetails = orderDetails.stream().filter(detail -> Objects.nonNull(detail.getProduct().getName())).collect(Collectors.toList());
		Order order = Order.builder().createdDate(new Date()).user(userBean.getLoginUser()).note(note).status(OrderStatus.IN_CREATED).orderDetails(orderDetails).build();
